...
export default function FaqError({ error, reset }: FaqErrorProps) {
  return (
    <div className="container mx-auto px-4 sm:px-6 py-12 sm:py-16 max-w-2xl">
      <Card className="shadow-xs border-destructive/20 bg-destructive/5 text-center py-8">
        <CardHeader className="flex flex-col items-center space-y-3">
          <div className="p-3 bg-destructive/10 rounded-full text-destructive">
            <AlertCircle className="h-8 w-8" />
          </div>
          <CardTitle className="text-xl sm:text-2xl font-bold text-foreground">
            Sıkça Sorulan Sorular Yüklenemedi
          </CardTitle>
          <CardDescription className="text-sm text-muted-foreground max-w-md">
            {error.message ||
              'SSS içeriklerine ulaşılırken sunucu kaynaklı bir sorun oluştu. Lütfen bağlantınızı kontrol edip tekrar deneyiniz.'}
          </CardDescription>
        </CardHeader>
